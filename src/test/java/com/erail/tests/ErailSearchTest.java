...
public class ErailSearchTest extends BaseTest {
    ...
    public void testErailStationSearchUseCase() throws Exception {
        Map<String, String> dataSet = JsonUtils.readTestDataSetByCaseName(
                ConfigReader.get("search.testdata.file"),
        ...
        Assert.assertTrue(stationsMatch,
                "Expected stations should match the first entries in the dropdown list");
        ...
        extentTest.pass("Step 8: Dynamically selected date " + daysAhead
                + " days ahead (" + expectedDateLabel + ")");

        Assert.assertTrue(
                selectedDate.toLowerCase().contains(
                        expectedDate.format(DateTimeFormatter.ofPattern("dd-MMM-yy", Locale.ENGLISH)).toLowerCase()),
                "Selected date should reflect " + daysAhead
                        + " days from today. Expected around: "
                        + expectedDateLabel + ", but was: " + selectedDate
        );
